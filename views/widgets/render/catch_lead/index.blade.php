        ...
        $sip_number =$aster->create_sip();  @endphp
                                <div class="on_of_nambers col-xs-12">
                                    <div class="form__block form__block_select_neiros">
                                        ...
                                        <select class="form-control select4-list select_sipNeiros" name="callback_tip[{{$item->id}}]">
                                            <option></option>
                                    ...
                                    <div class="form__block ">
                                        ...
                                        <input type="text" class="form-control class_hidenrte form-control-text <? if($item->callback_tip==1 || $item->callback_tip==2){ echo 'form-control-text-number';} else {echo 'form-control-text-number-mask';} ?>" name="phone_to_call[{{$item->id}}]"
                                               placeholder="+7 (___) ___ __ __" value="{{$item->phone_to_call}}"
                                        @if($item->callback_tip==2) style="display: none"@endif
                                        >
                                    </div>
                                    	<div class="form__block form__block_sip_neiros"  @if($item->callback_tip!=2) style="display:none" @endif>
                                            <div><span>Адрес:</span> pbx.neiros.ru</div>
                                            <div><span>Пользователь:</span> <span class="nom-neiros neiros_sip_login_1">@if(isset($sip_number['number'])){{$sip_number['number']}} @endif</span></div>
                                            <div><span>Пароль:</span> <span class="nom-neiros neiros_sip_password_1">@if(isset($sip_number['password'])){{$sip_number['password']}} @endif</span></div>
	</div>
                                    <div class="form__block form__block_small">
