        ...
        <div class="panel-body" style="padding-top:0px">
            ...
            <div class="cont-btn-content col-xs-12" style="padding-top: 0px;">
                <div class="tab-content-block active">
                    ...
                    <div id="sortable-panel-otobragenie-new">
                        ...
                        <div class="panel-otobragenie col-xs-12">
                            <form name="chat_form_operator_url"  >
                                ...
                                <div class="panel-header">
                                    ...
                                    <div class="block-header block-header-3 <? if($i_1 == 0){ echo 'active';} ?>"><img src="/images/icon/chat/3.png"></div>
                                    <div class="block-header block-header-4 block-header-4-new"><img src="/global_assets/images/icon_chat/file_gray.svg"></div>
                                    <div class="block-header block-header-5 <? if($i_1 == 0){ echo 'no-delete';} else {echo 'delete';}?>" data-id="{{$mess_rule->id}}"><img src="/global_assets/images/icon/user/trash.svg"></div>
                                ...
                                <div class="panel-body-panel col-xs-12" style="display: <? if($i_1 == 0){ echo 'block';} else {echo 'none';}?>;">
...
<? 
$condition = 'Выбрать правило';
if($rules->condition=='time-page') {
$condition = 'Время на текущей странице';
	} 
 if($rules->condition=='time-site') {
$condition = 'Время на сайте';
	 }
 if($rules->condition=='url-page') {
$condition = 'Адрес текущей страницы';
	 } 
 if($rules->condition=='return-user') {
$condition = 'Повторный пользователь';
	 } 
 
?>
                                                <div class="dropdown add-user-new add-number-new select-panel-blok-input">
                                                    <button  class="btn btn-success" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{$condition}}</button>
                                                    <ul class="dropdown-menu"  aria-labelledby="dLabel">
       ...
       <? 
$rules_condition = 'выберите условие';
if($rules->rules_condition=='%LIKE%') {
$rules_condition = 'содержит строку';
	} 
 if($rules->rules_condition=='NOT%LIKE%') {
$rules_condition = 'не содержит строку';
	 }
 if($rules->rules_condition=='==') {
$rules_condition = 'это точно';
	 } 
 if($rules->rules_condition=='!=') {
$rules_condition = 'не являеться';
	 } 
 
?>                                         <div class="dropdown add-user-new add-number-new select-panel-blok-input">
                                                    <button  class="btn btn-success" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">{{$rules_condition}}</button>
                                                    <ul class="dropdown-menu"  aria-labelledby="dLabel">
...
<div class="result__div_new" style="display:none"></div>
...
<script>
    ...
    $(document).on('click','#add_pravilo_operator',function(){
        ...
        $(this).closest('.tab-content-block').find('.show-chat-operator-all').append(`<div class="show-chat-operator col-xs-12">
                <div class=" operator-select-block">
                    <div class="dropdown operator-select add-user-new add-number-new select-panel-blok-input">
            <button  class="btn btn-success" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Выберите оператора</button>
             <ul class="dropdown-menu"  aria-labelledby="dLabel">
...
/*приветственное сообщение*/
    $(document).on('click','.add_sourse_block__chat',function(){
        ...
        $(this).closest('.tab-content-block').find('#sortable-panel-otobragenie-new').append(`<div class="panel-otobragenie col-xs-12">
    <form name="chat_form_operator_url"  >
        <input type="hidden" name="form_action" value="chat_mess_rules">
        <input type="hidden" name="widget" value="{{$widget->id}}">
        <input type="hidden" name="my_company_id" value="{{$widget->my_company_id}}">
 ...
 <div class="pravilo-container"></div>
		...
		$(document).on('click', 'form .block-header-4-new',function(){
		html = $(this).closest('.panel-otobragenie').html();
		Rand =	Date.now()
		$('.result__div_new').html(html)
		$('.result__div_new .block-header-5').removeClass('no-delete');
		$('.result__div_new .switchery-xs-new').html('<input checked type="checkbox" class="js-switch'+Rand+'" data-id="">')
		html = $('.result__div_new').html()
		
		$('#sortable-panel-otobragenie-new').append('<div class="panel-otobragenie col-xs-12">'+html+'</div>')
