...
if($cost->canal_id<=18){
if($cost->canal_id==\App\Servies\TypeTraffic::TYPE_ORGANIC){$canal_name='Органика';}
    if($cost->canal_id==\App\Servies\TypeTraffic::TYPE_REFERRAL){$canal_name='Переходы с сайтов';}
    if($cost->canal_id==\App\Servies\TypeTraffic::TYPE_SOCIAL){$canal_name='Социальные сети';}
    if($cost->canal_id==\App\Servies\TypeTraffic::TYPE_CPC){$canal_name='Рекламные системы';}
        }else{
 $wc=   \App\Models\WidgetCanal::find($cost->canal_id);
    if($wc){
       $canal_name=$wc->name;
    }
