<div class="info__intro">
    @if($tip==8)
        @if($is_manage==1)
            <a href="/ajax/telegramnotifi/{{$tg_user_id}}" id="telegramnotifi" class="btn telegramnotifi"  data-tg="{{$tg_user_id}}" target="_blank" >Отключить уведомления</a>

        @else
            <a href="/ajax/telegramnotifi/{{$tg_user_id}}" id="telegramnotifi" class="btn telegramnotifi"  data-tg="{{$tg_user_id}}" target="_blank" >Включить уведомления</a>
        @endif
