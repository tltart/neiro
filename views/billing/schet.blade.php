...
<head>@inject('Check','\App\Http\Controllers\CheckcompanysControllers')
    <title>Счет № {{$zakaz->number}} от {{date('d.m.Y',strtotime($zakaz->created_at))}}</title>
    ...
    <style>
        ...
        @font-face {
            font-family: 'arial';
            src: url({{ public_path('fonts/fonts/arial.ttf') }}) format('truetype');
            font-weight: 400;
            font-style: normal;
        }
        @font-face {
            font-family: 'arial_bold';
            src: url({{ public_path('fonts/fonts/arial-Bold.ttf') }}) format('truetype');
            font-weight: 400;
...
<body style="font-family:arial ">@php
...
<div style="font-weight: bold; font-size: 16pt; padding-left:5px;">
    Счет № {{$zakaz->number}} от {{date('d.m.Y',strtotime($zakaz->created_at))}}</div>
<br/>
