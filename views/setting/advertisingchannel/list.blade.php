 ...
 <table class="table tasks-list table-lg">
               ...
               @foreach($stages as $client)
                   <tr   id="del{{$client->id}}">
                    ...
                    <td>  @if($client->icon)
                            <img src="https://disk.neiros.ru/{{$client->icon}}" style="height:15px">
                        @endif{{$client->name}}</td>
                    <td>neiros={{$client->code}}</td>
                       <td>

                           @if(!$client->show_all)<a href="#" data-id="{{$client->id}}"  data-id="{{$client->id}}"  class="delet_chenel mkmkk" ><i class="glyphicon  glyphicon-trash" style="color: red;position: relative;"></i>
                               @else
                                   <i class="fa fa-lock" style="color: blue;position: relative;"></i>
                               @endif</a>
                       </td>
