...
/*END MODULE SOCKET */
var socket_neiros_io = io('neiros.cloud:6008/', {
    secure: true,
    query: "mk_id=ADMIN&site_id=" + admin_site,

});
socket_neiros_io.on("connect", () => {
    console.log('infoo');
});
socket_neiros_io.on('usersall', function (data) {
    ...
    for ( key in data.user) {
        $('.kkkl').append(`
        <tr class="m_` +data.user[key].neiros_visit + `">
        <td style="width: 200px">Клиент ` + data.user[key].mk_id + `<br>Сайт ` +data.user[key].site_id + `</td>
       
        <td>Метрика ` + data.user[key].metrika_id + `<br>
        NV ` + data.user[key].neiros_visit + `</td>
     
        <td><a href="` + data.user[key].url + `" target="_blank">` + data.user[key].title + ` </a>  </td>
      
        <td>` + data.user[key].ua + `<br> ` +data.user[key].ip +  `</td>
    ...
    socket_neiros_io.on('users', function (data) {

    /*array_clients ={
                mk_id:socket.handshake.query.mk_id,
                metrika_id:socket.handshake.query.metrika_id,
                session_id:socket.handshake.query.session_id,
                neiros_visit:socket.handshake.query.neiros_visit,
                url:socket.handshake.query.url,
                ua:socket.handshake.query.ua,
                site_id:socket.handshake.query.site_id,
    ...
    if (data.type == 'connected') {
        ...
        if (data.user.site_id == admin_site) {
            $('.kkkl').append(`
        <tr class="m_` + data.user.neiros_visit + `">
        <td style="width: 200px">Клиент ` + data.user.mk_id + `<br>Сайт ` + data.user.site_id + `</td>
       
        <td>Метрика ` + data.user.metrika_id + `<br>
        NV ` + data.user.neiros_visit + `</td>
     
        <td><a href="` + data.user.url + `" target="_blank">` + data.user.title + ` </a>  </td>
      
        <td>` + data.user.ua + `<br> ` + data.user.ip +  `</td>
    ...
    /*{
  "mk_id": "34",
  "metrika_id": "613715f0ba6071a8a37c4052",
  "session_id": "613715f0ba6071a8a37c4051",
  "neiros_visit": "612f47a8c8744c461082abe3",
  "title": "Быстровозводимый ангар склад в Мурманске | \"Smart Module\"",
  "ip": "::ffff:91.232.217.100",
  "id": "OEYE3gt7lZlBEduNAAAc"
}*/
